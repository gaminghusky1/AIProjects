...
def main():
    ...
    #
    # save_cifar_subset(train_images, train_labels, test_images, test_labels, 5, "cifar_subset_50.pkl")

    train_images, train_labels, test_images, test_labels = load_saved_subset("cifar_subset.pkl")
    ...
    k_vals = np.linspace(1.0, 20.0, 100)
    param_counts = [get_model_params(k) for k in k_vals]

    interp_k_from_param = interp1d(param_counts, k_vals, kind='linear', fill_value="extrapolate")

    target_params = np.linspace(1e6, 25e6, num=15)  # Adjust num for how many models you want

    target_ks = interp_k_from_param(target_params)
    print(target_ks)

    # ~50M to 100M
    # target_ks = [12.47770686, 12.91031841, 13.33075301, 13.74687293, 14.14109097, 14.52939182,
    #              14.9075623, 15.27765392, 15.63747105, 15.98180775, 16.33031169, 16.66686589,
    #              16.99899094, 17.31550055, 17.63794865]

    # ~1M to 50M
    # target_ks = [ 1.76316614,  3.74639299,  4.99537563,  5.98339418,  6.83460037,  7.58984969,
    #               8.27671103,  8.9093379,   9.50092525, 10.05582353, 10.58114026, 11.09041324,
    #               11.56714405, 12.02648567, 12.47770686]

    models = [build_model(k) for k in target_ks]

    for model in models:
        model.compile(optimizer='adam', loss='categorical_crossentropy')

        model.fit(train_images, ohe_train_labels, batch_size=25, epochs=100, verbose=0)
    ...
    plt.figure()
    sns.swarmplot(x=residuals, size=6)
    plt.yticks([])
    plt.xlabel('Residual')
    plt.title('Dotplot of Residuals')
    plt.savefig("Resid Dot.png")
    plt.show()
