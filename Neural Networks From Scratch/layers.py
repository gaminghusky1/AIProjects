...
class Dense:
    ...
    def init_weights(self, last_layer_shape):
        self.weights = np.random.randn(np.prod(last_layer_shape), self.units) * np.sqrt(1 / np.prod(last_layer_shape))
    ...
    def forward_pass(self, prev_layer_activations):
        prev_layer_activations = prev_layer_activations.flatten()
    ...
    def backward_pass(self, prev_layer_activations, curr_layer_z, dc_da):
        ...
        if self.activation.elementwise:
            ...
            dc_dz = np.dot(dc_da, self.activation.derivative(curr_layer_z))
        # Cost of current layer weights
        # dc_dw; dz_dw = a_L-1
        self.weights_gradient += dc_dz * prev_layer_activations[:, np.newaxis]
        # Cost of biases
        # dz_db = 1 because biases are constants
        self.biases_gradient += dc_dz
...
class Convolution:
    ...
    def init_weights(self, last_layer_shape):
        ...
        self.kernel_stack_shape = (last_layer_shape[0], *self.kernel_shape)
    ...
    def forward_pass(self, prev_layer_activations):
        prev_layer_activations = prev_layer_activations.reshape(self.input_shape)
        ...
        z_output = np.zeros(self.stacked_output_shape)
        for f in range(self.num_filters):
    ...
    def backward_pass(self, prev_layer_activations, curr_layer_z, dc_da):
        prev_layer_activations = prev_layer_activations.reshape(self.input_shape)
        curr_layer_z = curr_layer_z.reshape((self.num_filters, self.output_size))
        dc_da = dc_da.reshape((self.num_filters, self.output_size))
        prev_layer_activations = np.pad(prev_layer_activations, ((0, 0), (self.padding, self.padding), (self.padding, self.padding)), mode='constant', constant_values=0)
        da_dz = self.activation.derivative(curr_layer_z)
        if self.activation.elementwise:
            dc_dz = dc_da * da_dz
        else:
            dc_dz = np.einsum('fi,fij->fj', dc_da, da_dz)
        ...
        dc_dw = np.einsum('fo,fko->fk', dc_dz, dz_dw)
        self.weights_gradient += dc_dw.reshape(self.num_filters, *self.kernel_stack_shape)
        self.biases_gradient += np.sum(dc_dz, axis=1)

        dc_da = np.einsum('fo,foi->i', dc_dz, self.dz_da)
        return dc_da
...
class MaxPooling:
    ...
    def forward_pass(self, prev_layer_activations):
        ...
        return a_output, max_indices

    def backward_pass(self, prev_layer_activations, max_indices, dc_da):
        prev_layer_activations = prev_layer_activations.reshape(self.input_shape)
